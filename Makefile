BINARY_NAME = nanobanana-server
BUILD_FLAGS = -trimpath -ldflags="-buildid="
# Reference Go version for reproducible builds: 1.25.0
GO_VERSION = 1.25.0
...
docker-reproducible-build:
	...
	docker cp $(BINARY_NAME)-temp:/app/$(BINARY_NAME)-linux-amd64 ./dist/
	docker cp $(BINARY_NAME)-temp:/app/$(BINARY_NAME)-linux-arm64 ./dist/
	docker cp $(BINARY_NAME)-temp:/app/$(BINARY_NAME)-windows-amd64.exe ./dist/
	docker cp $(BINARY_NAME)-temp:/app/$(BINARY_NAME)-darwin-amd64 ./dist/
	docker cp $(BINARY_NAME)-temp:/app/$(BINARY_NAME)-darwin-arm64 ./dist/
	# Also copy Linux binary to root for compatibility with other targets
	docker cp $(BINARY_NAME)-temp:/app/$(BINARY_NAME)-linux-amd64 ./$(BINARY_NAME)
